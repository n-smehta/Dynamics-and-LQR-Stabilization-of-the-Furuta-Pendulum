...
struct EncoderState read_cart_state()
{
    ...
    cart_state.angle = (position/( ENCODER_COUNT-1))*M_PI;
    cart_state.angle_velocity = (velocity/( ENCODER_COUNT-1))*M_PI;
...
struct EncoderState read_pendulum_state()
{
    ...
    pendulum_state.angle = (position/( ENCODER_COUNT-1))*M_PI;
    pendulum_state.angle_velocity = (velocity/( ENCODER_COUNT-1))*M_PI;
